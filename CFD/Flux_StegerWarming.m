function [Fplus, Fminus, Aplus, Aminus] = Flux_StegerWarming(U, gamma)
Fplus = zeros(3,1); Fminus = zeros(3,1);
[rho, u, ~, c] = conservative_to_primitive(U, gamma);
lambda1 = u; lambda2 = u+c; lambda3 = u-c;
lambda1p = (lambda1+sqrt(lambda1^2+1e-6))/2; lambda1m = (lambda1-sqrt(lambda1^2+1e-6))/2;
lambda2p = (lambda2+sqrt(lambda2^2+1e-6))/2; lambda2m = (lambda2-sqrt(lambda2^2+1e-6))/2;
lambda3p = (lambda3+sqrt(lambda3^2+1e-6))/2; lambda3m = (lambda3-sqrt(lambda3^2+1e-6))/2;
Aplus = diag([lambda1p,lambda2p,lambda3p]);
Aminus = diag([lambda1m,lambda2m,lambda3m]);
